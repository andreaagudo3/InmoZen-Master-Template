...
export function ContactChannels({ propertyTitle }) {
  ...
  const hasWhatsApp = tenant?.effective_features?.whatsappButton && phones.length > 0
  const hasPhone = phones.length > 0
  const isDemo = tenant?.isDemoMode

  if (!hasWhatsApp && !hasPhone) return null

  const rawPhone = phones[0]?.replace(/\s+/g, '') ?? ''
  const url = typeof window !== 'undefined' ? window.location.href : ''

  const message = propertyTitle
    ? t('whatsapp.msgProperty', { title: propertyTitle, url })
    : t('whatsapp.msgGeneral')
