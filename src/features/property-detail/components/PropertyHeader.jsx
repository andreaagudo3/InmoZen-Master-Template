...
export function PropertyHeader({ title, listing_type, price, status, locationString }) {
  ...
  return (
    <header>
      ...
      {locationString && (
        tenant?.effective_features?.googleMaps ? (
          <a
            ...
            className="inline-flex items-center gap-1 text-secondary-500 hover:text-primary-700 hover:underline underline-offset-2 transition-colors cursor-pointer w-fit"
          >
          ...
          <span className="inline-flex items-center gap-1 text-secondary-500 w-fit">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-secondary-400 shrink-0" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
            </svg>
            {locationString}
          </span>
      ...
      {price != null && (
        <p className="text-4xl font-extrabold text-secondary-950 mt-4">
          {formatPrice(price)}
          {listing_type === 'rent' && <span className="text-lg font-normal text-secondary-500 ml-1">{t('header.perMonth', '/mes')}</span>}
        </p>
      )}
    </header>
  )
